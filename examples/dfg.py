...
def dot_to_pkl(dotfile, pklfile):
    ...
    with open(pklfile, 'wb') as f:
        pk.dump(graph_info, f)

def check_attributes(dotfile):
    graph = pgv.AGraph(dotfile)
    print("Graph Attributes:", graph.graph_attr)
    print("Node Attributes:", graph.node_attr)
...
def main():
    ...
    for name, bindlist in binddict.items():
        for bind in bindlist:
            graphgen.generate(str(bind.dest), walk=options.walk, identical=options.identical,
                        step=options.step, do_reorder=options.reorder, delay=options.delay, alwaysinfo=bind.alwaysinfo, withcolor=True)
    ...
    pkl_filename = './out/pkl/graph.pkl'
    dotfile = './file.dot'
    #dot_to_pkl(dotfile=dotfile,pklfile=pkl_filename)
    #print(graphgen.graph.to_string())
    with open(pkl_filename,'wb') as f:
        pk.dump(graphgen.graph.to_string(),f)
    #check_attributes(dotfile)
    graphgen.draw()
